...
def addHours():
    ...
    if (open_time):
        ...
        if (start >= end):
            return {
                "error": "End cannot be greater than start!"
            }
...
def setAvailability():
    ...
    # Set availability
    err = avail.set(start_time=start_time, end_time=end_time,
...
def addSchedule():
    ...
    if request.method == 'GET':
        ...
        jsonres = request.get_json()
        token = jsonres.get('token')
        employee_id = jsonres.get('emp_id')
        date = jsonres.get('date')
        start_time = jsonres.get('start_time')
        end_time = jsonres.get('end_time')
    if (token is None or employee_id is None or date is None or start_time is None or end_time is None):
        return {
            "error": "Value cannot be null!"
        }
    ...
    try:
        if (date):
            date = datetime.fromisoformat(str(date).split("T")[0])
        if (start_time):
            if (len(start_time) == 5):
                start_time = start_time + ":00"
            if (len(end_time) == 5):
    ...
    err = schedule.get(employee_id, date)
    if (not err):
        err = schedule.set(start_time, end_time)
        if (err):
            return err
        else:
            return {
                "success": "Successfully updated schedule."
            }

    # Add schedule
    err = schedule.add(employee_id, date, start_time, end_time)
    if (err):
        return {
            "error": err
        }
        ...
        return {
            "success": "Successfully added schedule."
        }
...
def getSchedule():
    ...
    if request.method == 'GET':
        ...
        jsonres = request.get_json()
        token = jsonres.get('token')
        employee_id = jsonres.get('employee_id')
        date = jsonres.get('date')
    if (token is None or employee_id is None or date is None):
        return {
            "error": "Value cannot be null!"
        }
    ...
    if (err):
        ...
        return json.dumps(schedule.to_dict(), default=str)


@app.route('/api/deleteSchedule', methods=['GET', 'POST'])
def deleteSchedule():
    # Get params
    if request.method == 'GET':
        token = request.args.get('token')
    ...
    if (err):
        ...
        return {
            "success": "Successfully deleted schedule item."
        }


@app.route('/api/getSchedules', methods=['GET', 'POST'])
def getSchedules():
    # Get params
    if request.method == 'GET':
        token = request.args.get('token')
        ...
        token = jsonres.get('token')
        date = jsonres.get('date')
    if (token is None or date is None):
        return {
            "error": "Value cannot be null!"
        }
    ...
    # Get schedule
    schedules = [sch.to_dict() for sch in user.store.getSchedule(date)]
    return json.dumps({"schedules": schedules}, default=str)
